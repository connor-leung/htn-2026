...
Factories: `label box bar arc slider image line button switch checkbox roller textarea`. (`line` points are `{{x,y},…}` or `{{x=…,y=…},…}`,
...
or `small default large`. Selectors are `main indicator knob items scrollbar`, optionally with `:pressed :checked :disabled :focused`.
...
`badge.ui.theme` mirrors the firmware theme: `background panel surface track border accent accent_detail text text_soft text_muted text_dim`.
...
and rescan, not successful app initialization.** Likewise, an `app_reg: launched` line can follow a Lua startup error: inspect the preceding
