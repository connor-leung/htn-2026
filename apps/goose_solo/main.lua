-- Goose Solo - a Waterloo goose battler for the 2026 Hacker Badge.
--
-- Controls
...
--
-- This is the single-badge build of Goose Duel. The radio, the pairing
-- protocol and the wake lock are gone: nothing here transmits, and the badge
-- is free to sleep between ticks. See ../../main.lua for the duelling build.
...
-- ---------------------------------------------------------------- state ---
-- ST_PASS is the hand-over screen: two players share one badge, so each
-- player's move has to be hidden from the other between choices.
...
local hotseat = false    -- two players, one badge
local active = 1         -- whose turn it is to choose, in a hot-seat duel
local picking_p2 = false -- the goose picker is choosing player 2's goose
local turn = 1
local my_move, foe_move
local resolve_at = 0     -- delay before showing the exchange
...
local function render_moves()
  -- Whoever is choosing sees their own moves. On the hand-over screen nobody
  -- sees anything: that is the entire point of it.
...
local function finish(won)
  ...
  st = ST_RESULT
  if hotseat then
    -- A friend's duel does not touch the badge owner's record or XP: the
    -- other player's goose is not theirs to win or lose progress with.
  ...
  -- 250 ms budget is shared with tick work. The write happens on the
...
local function choose_move()
  if hotseat then
    if active == 1 then
      ...
      move_sel = 1
      st = ST_PASS
      render_moves()               -- blanks the list; P2 must not see P1's pick
...
function on_button(button, kind)
  ...
  if st == ST_MENU then
    ...
    elseif button == B.A then
      if menu_sel == 1 then
        ...
        -- Both remaining items are the goose picker; only the reason differs.
  ...
  elseif st == ST_PICK then
    ...
    elseif button == B.A then
      if picking_p2 then
        -- Player 2 fights at player 1's level, so the duel is about the
        -- matchup rather than who has played more.
        hotseat = true
        rng = badge.sys.random(65536) % 65537
  ...
  elseif st == ST_BATTLE then
    ...
    -- In a hot-seat duel my_move is already set while player 2 chooses, so
    -- it must not lock the controls the way it does against the CPU.
  ...
  elseif st == ST_RESULT then
    ...
    if button == B.A then
      rng = badge.sys.random(65536) % 65537
      if hotseat then
        start_battle(foe.sp, save.level)   -- same two geese, straight rematch
      else
        start_battle(badge.sys.random(#GEESE) + 1,
          save.level > 2 and save.level - 1 or 1)
      end
